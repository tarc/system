...
![system build](https://github.com/tarc/system/workflows/system%20build/badge.svg)
...
```nix
{ config, pkgs, ... }: {
  home-manager.users.tarc = import ./home-manager/home.nix;
}
```
...
```bash
sudo nixos-install --flake github:tarc/system#phil
```
...
```bash
git clone https://github.com/tarc/system ~/.nixpkgs
```
